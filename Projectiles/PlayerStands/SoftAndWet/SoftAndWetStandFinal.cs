...
namespace JoJoStands.Projectiles.PlayerStands.SoftAndWet
{
    public class SoftAndWetStandFinal : StandClass
    {
        ...
        public override void AI()
        {
            ...
            if (mPlayer.standControlStyle == MyPlayer.StandControlStyle.Manual)
            {
                ...
                if (bubbleMode)
                {
                    ...
                    if (bubbleSpawnTimer >= 45)
                    {
                        ...
                        int projIndex = Projectile.NewProjectile(Projectile.GetSource_FromThis(), bubbleSpawnPosition, bubbleVelocity, ModContent.ProjectileType<ControllableBubble>(), (int)((AltDamage + 42) * mPlayer.standDamageBoosts), 2f, Projectile.owner);
                        Main.projectile[projIndex].netUpdate = true;
            ...
            if (amountOfBubbleBombs > 0 && MyPlayer.AutomaticActivations)
            {
                int numberProjectiles = 4;
                float rotation = MathHelper.ToRadians(55);
                float randomSpeedOffset = Main.rand.NextFloat(-3f, 3f);
