    ...
    public class CreamStandFinal : StandClass
    {
        ...
        public override void AI()
        {
            ...
            if (mPlayer.standControlStyle == MyPlayer.StandControlStyle.Manual && !mPlayer.creamDash)
            {
                ...
                {
                    if (PlayerLeftClick() && !mPlayer.creamVoidMode && !mPlayer.creamExposedMode && !mPlayer.creamExposedToVoid && !mPlayer.creamNormalToExposed && !mPlayer.creamDash)
                    {
                        attacking = true;
            ...
            if (mPlayer.creamExposedToVoid || mPlayer.creamVoidMode || mPlayer.creamExposedMode)
            {
                ...
                behindPlayer.X += (float)((player.width / 2) * player.direction);
                behindPlayer.Y -= -35 + HalfStandHeight;
                Projectile.Center = Vector2.Lerp(Projectile.Center, behindPlayer, 1f);
