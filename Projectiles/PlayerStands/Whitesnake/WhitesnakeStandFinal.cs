...
namespace JoJoStands.Projectiles.PlayerStands.Whitesnake
{
    public class WhitesnakeStandFinal : StandClass
    {
        ...
        private const float SleepingGasEffectRadius = 15f * 16f;
        ...
        public override void AI()
        {
            ...
            if (mPlayer.standControlStyle == MyPlayer.StandControlStyle.Manual)
            {
                ...
                {
                    if (PlayerLeftClick() && !secondaryAbility && !stealingDisc)
                    {
                ...
                if (!attacking && !stealingDisc && !waitingForStealEnemy && !revealingGun && waitingForEnemyFramesButInt == 0)
                {
                    if (!secondaryAbility)
                        StayBehind();
                    else
                ...
                if (SpecialKeyCurrent() && shootCount <= 0 && !stealingDisc)
                {
                ...
                if (!SpecialKeyCurrent() && stealingDisc || !SpecialKeyCurrent() && waitingForStealEnemy || Projectile.ai[1] != -1f)
                {
                    stealingDisc = false;
                ...
                if (SecondSpecialKeyPressed(false) && shootCount <= 0 && !revealingGun && !stealingDisc)
                {
            ...
            if (sleepingGasTimer != 0)
            {
                ...
                for (int n = 0; n < Main.maxNPCs; n++)
                {
                    ...
                    if (npc.active && npc.Distance(sleepingGasPosition) <= SleepingGasEffectRadius)
                    {
                        ...
                        npc.velocity.X *= 0.4f - slowBonus;
                        if (npc.noGravity)
                            npc.velocity.Y *= 0.4f - slowBonus;
                    }
                }
        ...
        {
            stealingDisc = reader.ReadBoolean();
