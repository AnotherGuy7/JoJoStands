...
namespace JoJoStands.Projectiles.PlayerStands.TowerOfGray
{
    public class TowerOfGrayStandT3 : StandClass
    {
        ...
        private const float ManualModeRange = 22 * 16;
        private const float RemoteModeRange = 66 * 16;
        ...
        public override void AI()
        {
            ...
            if (!returnToPlayer && !returnToRange) // basic stand control
            {
                ...
                {
                    if (PlayerLeftClick() && !dashing && targetWhoAmI == -1 && Projectile.owner == Main.myPlayer)
                    {
                    ...
                    if (SecondSpecialKeyPressed() && Projectile.owner == Main.myPlayer) // targetWhoAmI ability activation
                    {
                        arrayClear = true;
                        for (int n = 0; n < Main.maxNPCs; n++)
                        {
                            NPC npc = Main.npc[n];
                            if (npc.active && !npc.hide && !npc.immortal && Projectile.Distance(npc.Center) <= SpecialDetectionRange && !npc.friendly & npc.lifeMax > 5)
                ...
                if (dashing && Projectile.owner == Main.myPlayer) //right click abilty
                {
                ...
                if (!dashing && targetWhoAmI == -1) //return to stand controlRange after ability
                {
            ...
            if (Projectile.Distance(player.Center) <= controlRange * 0.9f && returnToRange) //if suddenly stand is out of controlRange
            {
                MovementAI(player.Center, 0f);
                returnToRange = false;
            }

            if (Projectile.Distance(player.Center) >= 1000f && !dashing && !returnToPlayer && targetWhoAmI == -1 && !remoteMode) //if suddenly stand is too far
            {
                arrayClear = true;
                returnToPlayer = true;
            }
