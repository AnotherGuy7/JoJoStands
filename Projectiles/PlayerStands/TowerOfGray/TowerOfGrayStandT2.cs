...
namespace JoJoStands.Projectiles.PlayerStands.TowerOfGray
{
    public class TowerOfGrayStandT2 : StandClass
    {
        ...
        private float remoteRange = 900f; //750f 900f 1050f 1200f
        ...
        public override void AI()
        {
            ...
            if (mPlayer.usedEctoPearl && noRemoteRange == 300f)
                noRemoteRange *= 1.5f;
            if (mPlayer.usedEctoPearl && remoteRange == 900f)
                remoteRange *= 1.5f;
            ...
            if (!returnToPlayer && !returnToRange) // basic stand control
            {
                ...
                if (mPlayer.standControlStyle == MyPlayer.StandControlStyle.Auto && !dash) // automode attack ai (only half of original damage)  
                {
                    remoteMode = false;
                    if (target != null)
                    {
