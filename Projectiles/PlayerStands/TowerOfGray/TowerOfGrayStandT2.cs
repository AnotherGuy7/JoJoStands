...
namespace JoJoStands.Projectiles.PlayerStands.TowerOfGray
{
    public class TowerOfGrayStandT2 : StandClass
    {
        ...
        public override void AI()
        {
            ...
            if (!dashing && !returnToPlayer && !returnToRange)
            ...
            if (!returnToPlayer && !returnToRange) // basic stand control
            {
                ...
                if (mPlayer.standControlStyle == MyPlayer.StandControlStyle.Auto && !dashing) // automode attack ai (only half of original damage)  
                {
                    remoteMode = false;
                    if (target != null)
                    {
                ...
                if (!dashing) //return to stand controlRange after ability
                {
            ...
            if (Projectile.Distance(player.Center) >= 1000f && !dashing && !returnToPlayer && !remoteMode) //if suddenly stand is too far
            {
                arrayClear = true;
                returnToPlayer = true;
            }
